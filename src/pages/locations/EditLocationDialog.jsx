...
export default function EditLocationDialog({
  ...
  useEffect(() => {
    ...
    setLocationName(currentItem?.location || '');
    setSaving(false);
    setConfirmOpen(false);
  }, [open, currentItem]);
  ...
  const validate = () => {
    if (!currentItem?.id) return 'Invalid location item';
  ...
  const handleConfirm = async () => {
    ...
    try {
      await axios.put(
        `${LOCATION_API}/${currentItem.id}`,
        {
          id: currentItem.id,
          location: locationName.trim(),
          userIdCreate: currentItem.userIdCreate,
      ...
      console.error(err);
      toast(err?.response?.data?.message || 'Update location failed', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
            ...
            >
              <Stack direction="row" spacing={1}>
                ...
                <Typography fontSize={12}>
                  Editing location only updates the location name. Creator and created date are kept unchanged. Backend may block rename if the location is already used.
                </Typography>
              </Stack>
            </Box>
          </Stack>
      ...
      <Dialog open={confirmOpen} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Typography>
            Update location <b>{locationName}</b> ?
          </Typography>
        </DialogContent>
